...
def invoke_task(
    ...
    for input_name in task.inputs.keys():
        if input_name not in input_locations.keys():
            raise ValueError(
                f"This task is supposed to receive a pointer to an input named '{input_name}'. However, only the following input pointers were supplied: {list(input_locations.keys())}"
            )

    for output_name in task.outputs.keys():
        if output_name not in output_locations.keys():
            raise ValueError(
                f"This task is supposed to receive a pointer to an output named '{output_name}'. However, only the following output pointers were supplied: {list(output_locations.keys())}"
            )
