...
def test__from_function_return_value__with_dict__and_missing_key():
    ...
    assert (
        str(e.value)
        == "An output of type FromKey('x') expects the return value of the function to be a dictionary containing, at least, a key named 'x'"
    )
...
def test__from_function_return_value__with_unsupported_type():
    ...
    assert (
        str(e.value)
        == "This output is of type FromKey. This means we expect the return value of the function to be a dictionary containing, at least, a key named 'x'"
    )
